...
for item, data in tqdm(input_dict.items()):
    output.append((item, prd2.get_prediction(item, data, items, all_item_data, sim_matrix, user_avgs)))
...
predictions_path = os.path.join(local_dir, "..", "..", "Data", "Output", "counts-" + db_name + ".csv")
